...
- **Download confirmation.** Before `download` writes a file to the server's disk (i.e. not `resolve_only` and not a remote/HTTP server), an elicitation-capable client is asked to confirm, showing the destination file name and, best-effort, its size. Declining writes nothing and returns the resolved direct link instead, so you can fetch it yourself.
