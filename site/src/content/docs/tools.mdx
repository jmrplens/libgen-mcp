...
<Aside type="note">
	...
	degrade silently to an absent `enrichment` field. Enrichment runs synchronously
	within the `get_details` call: it never fails the core result, but it may add
	bounded latency — up to the ~6s budget — before the response returns. No API
	key is required; both APIs are used keyless.
</Aside>
...
<Aside type="note">
	Provide exactly one of `md5`, `doi`, or `path`. A `path` on a remote server
	(`--http`, or a stdio server with `LIBGEN_MCP_REMOTE_DOWNLOADS=1`) is rejected
	— the server cannot see the client's filesystem.
</Aside>
