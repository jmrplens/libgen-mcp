...
Provide exactly one of `md5` or `id`. Supplying both, neither, an `md5` that is not 32 hex chars, or an `object` other than `edition`/`file` returns an input error.
...
- **Both `md5` and `doi`** → article sources first (`unpaywall`, `sci-hub`), then book sources (`libgen`, `randombook`).
