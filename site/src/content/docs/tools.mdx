...
When the connected MCP client supports **elicitation** (asking you for input mid-call), `download` may pause to ask two things — both purely opt-in, and both no-ops on a client that doesn't advertise the capability:
...
<Aside type="note">
	Neither prompt is ever required: elicitation is entirely opt-in and
