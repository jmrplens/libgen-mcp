...
<Aside type="note">
	...
	degrade silently to an absent `enrichment` field. Enrichment never fails the
	`get_details` call and never delays it beyond its own budget — the core
	response is unaffected either way. No API key is required; both APIs are used
	keyless.
</Aside>
