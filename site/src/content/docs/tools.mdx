...
Full metadata for a record via the LibGen JSON API — description, identifiers, DOI, cover, and related edition. Look up by `md5`, by `id`, or by `doi` — exactly one of the three.
...
Download a file to a local directory. Provide `md5` or `isbn` for a book **or** `doi` for an article; at least one is required. Returns the saved path, size, and the source that served it.
