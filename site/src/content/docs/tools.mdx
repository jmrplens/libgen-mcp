...
- **Article** (`doi` only) → `unpaywall` (open-access PDF; only when `LIBGEN_MCP_UNPAYWALL_EMAIL` is set, otherwise skipped), then `scihub`, then `scidb` (Anna's Archive SciDB viewer).
- **Both `md5` and `doi`** → article sources first (`unpaywall`, `scihub`, `scidb`), then book sources (`libgen`, `randombook`, `annas`).
