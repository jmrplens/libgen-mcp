        ...
        "mainEntity": [
          {
            ...
            "acceptedAnswer": {
              "@type": "Answer",
              "text": "Not unless you turn it on, and never to the maintainer. There is no analytics, no crash reporting and no backend of its own; the server creates no database and no telemetry file, and logs to standard error, where your MCP client collects them if it collects them at all. The maintainer never receives your queries, your downloads or any usage information, whatever you configure. LIBGEN_MCP_TELEMETRY lets you export OpenTelemetry traces, metrics and logs to a collector you run; it is off by default, the exporters' only default destination is your own localhost, and what they carry describes operations rather than what was searched for. See OpenTelemetry, if you turn it on."
            }
          },
          {
            "@type": "Question",
