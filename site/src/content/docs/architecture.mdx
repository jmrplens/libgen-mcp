...
Download sources implement a common interface — `Name`, `Supports(item)`, and `Resolve(ctx, item)` — so the shared pipeline stays provider-agnostic. The chain is built from configuration in the fixed order `unpaywall → europepmc → biorxiv → fatcat → core → scihub → scidb → libgen → randombook → annas`, and each source is offered only the items it supports:
