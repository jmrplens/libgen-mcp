...
<Aside>
	...
	degradan en silencio a un campo `enrichment` ausente. El enriquecimiento nunca
	hace fallar la llamada a `get_details` ni la retrasa más allá de su propio
	presupuesto — la respuesta principal no se ve afectada. No se requiere clave
	de API; ambas APIs se usan sin autenticación.
</Aside>
