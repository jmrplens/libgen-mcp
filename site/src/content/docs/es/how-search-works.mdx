...
- [arXiv](https://arxiv.org/), [Crossref](https://www.crossref.org/) y [OpenLibrary](https://openlibrary.org/) — proveedores de acceso abierto. Sus resultados se identifican por DOI (o ISBN para OpenLibrary), así que van a su propia lista.
...
- **`always`.** Consultar las fuentes adicionales en cada búsqueda. Como esta decisión no depende de lo que responda el catálogo, las fuentes adicionales se consultan *a la vez* que el catálogo y no después, así que una búsqueda forzada cuesta una vuelta de latencia en lugar de dos. Útil cuando quieres específicamente la red más amplia, a costa de tráfico extra en cada llamada.
...
- **`libgen`** y **`annas`** — el resultado se identifica por un md5. Descárgalo con el argumento `md5` de `download`.
- **`arxiv`**, **`crossref`** y **`openlibrary`** — el resultado se identifica por un DOI. Descárgalo con el argumento `doi` de `download`.
...
flowchart LR
    ...
    O -->|annas| M
    O -->|arxiv / crossref<br/>/ openlibrary| P[tiene un DOI<br/>→ descargar por doi]
```
