...
description: "Cómo se prueba libgen-mcp: un arnés controlado conduce a un LLM real sobre MCP contra el sitio en vivo y evalúa lo que hace. Resultados de la última ejecución completa."
...
Las dos llamadas con error de la tabla son deliberadas: un escenario fija un host
de Sci-Hub muerto para comprobar que el calendario de reintentos devuelve un error
accionable, y otro restringe el despliegue al catálogo para comprobar que
`extra_sources: never` aguanta.
