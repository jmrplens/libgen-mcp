...
head:
  ...
  # Resultados de medición, así que Dataset es el tipo correcto. Sin
  # `distribution`: las cifras se generan en esta página desde el informe de la
  # ejecución y no hay ningún artefacto legible por máquina al que apuntar.
  - tag: script
    attrs:
      type: application/ld+json
    content: |
      {
        ...
        "measurementTechnique": "Sesión MCP real de extremo a extremo contra un modelo de lenguaje real, mirrors reales y descargas reales",
        "variableMeasured": [
          {
            "@type": "PropertyValue",
