...
Cuarenta y un escenarios cubren búsqueda, detalles, descargas, selección de
...
**37 pasaron, 1 falló, 3 en SKIP** (de 41 incluida la variante `S6b`).

Los tres SKIP son condiciones de datos en vivo (un HTTP 403, un PDF escaneado sin
capa de texto, un contenedor que el extractor no soporta) — nunca un fallo del
modelo ni del servidor. El único fallo, S29, es un hallazgo real sobre el
comportamiento del modelo y no un fallo del servidor: el modelo sí fijó
`extra_sources` y sí recibió resultados de acceso abierto, pero su respuesta no
citó ningún identificador de ellos. Se deja en rojo a propósito: un arnés que mide
lo bien que un modelo usa las herramientas no debe ajustarse hasta darse la razón
a sí mismo.

S41 (el nivel de socio de Anna's) no aparece en la tabla: el proceso del arnés
terminó durante su ejecución, así que no hay resultado que reportar. Una corrida
anterior sí llegó a él y quedó en SKIP, al fallar en vivo la descarga de socio.
