...
{/* prettier-ignore */}
La suite son **69 escenarios** (S1–S68 más la variante S6b). 10 de ellos ejercitan un servidor en modo remoto (`--http`); el resto lo ejecutan sobre stdio.
...
{/* prettier-ignore */}
La tabla siguiente es un conjunto de ejecuciones en vivo entre el 2026-07-25 y el 2026-07-30 contra `claude-haiku-4-5-20251001` (API real de Anthropic, mirrors reales, descargas reales): **69 pasaron, 0 fallaron, 0 en SKIP** de 69 — todos los escenarios, incluidos los 10 que se ejecutan contra un servidor en modo remoto (`--http`). Cada fila lleva la fecha en que se midió por última vez: una ejecución parcial solo refresca los escenarios que ejecutó.
