...
{/* prettier-ignore */}
La suite son **78 escenarios** (S1–S77 más la variante S6b). 10 de ellos ejercitan un servidor en modo remoto (`--http`); el resto lo ejecutan sobre stdio.
...
{/* prettier-ignore */}
La tabla siguiente es una única ejecución en vivo de la suite completa el 2026-07-30 contra `claude-haiku-4-5-20251001` (API real de Anthropic, mirrors reales, descargas reales): **76 pasaron, 0 fallaron, 0 en SKIP** de los 76 medidos hasta ahora, incluidos los 10 que se ejecutan contra un servidor en modo remoto (`--http`). 2 escenarios de la lista anterior aún no tienen fila aquí: se añadieron después de la última ejecución en vivo, y un resultado solo se publica una vez medido.
