...
Las descargas usan un cliente HTTP aparte **sin timeout global** — las transferencias largas están acotadas por el contexto de la petición, no por un plazo fijo. La concurrencia está limitada por un semáforo de tamaño `LIBGEN_MCP_MAX_CONCURRENT_DOWNLOADS`; las descargas extra hacen cola por un hueco (y pueden cancelarse mientras están en cola, antes de tocar la red).

Para un elemento dado, la tubería:
...
3. **Obtiene** el archivo; si ya hay bytes en disco, envía una cabecera `Range` para **reanudar** desde ese offset.
...
5. **Valida** la respuesta — rechaza páginas HTML de error (por `Content-Type` y olfateando los primeros 512 bytes), aplica el límite de tamaño contra el tamaño total esperado y comprueba el espacio libre en disco.
6. **Transmite** el cuerpo al archivo `.part` mientras calcula su MD5 e informa progreso estrangulado. En reanudación, vuelve a calcular el hash de los bytes ya en disco para que el digest final cubra todo el archivo.
...
Las fuentes de descarga implementan una interfaz común — `Name`, `Supports(item)` y `Resolve(ctx, item)` — de modo que la tubería compartida permanece agnóstica del proveedor. La cadena se construye desde la configuración en el orden fijo `unpaywall → scihub → libgen → randombook`, y a cada fuente solo se le ofrecen los elementos que admite:
