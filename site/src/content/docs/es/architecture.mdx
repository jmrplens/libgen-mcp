...
| `core`       | DOI       | Artículos de acceso abierto   | Busca el DOI en la API v3 de CORE (autenticada por Bearer) y devuelve la URL de descarga del trabajo. Opcional: ausente de la cadena salvo que `LIBGEN_MCP_CORE_KEY` esté fijado. La clave se envía solo a `api.core.ac.uk`.                                                                                          |
