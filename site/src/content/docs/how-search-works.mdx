...
- [arXiv](https://arxiv.org/), [Crossref](https://www.crossref.org/), and [OpenLibrary](https://openlibrary.org/) — open-access providers. Their hits are not md5-keyed, so they land in their own list, each carrying one actionable identifier: a DOI from Crossref, a direct `pdf_url` from arXiv, and an `isbn`/title from OpenLibrary.
...
- **`libgen`** and **`annas`** — the result carries an md5. Download it with `download`'s `md5` argument.
