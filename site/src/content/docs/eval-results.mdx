...
A clean sweep is worth reading carefully rather than celebrating. It took removing
the two things that were hiding trouble: scenarios that skipped whenever live data
...
The two errored tool calls in the table are deliberate: one scenario pins a dead
host to prove the retry schedule surfaces an actionable error, and another
restricts the deployment to the catalog to prove the restriction holds.
