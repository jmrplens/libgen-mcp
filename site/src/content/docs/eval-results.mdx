...
Forty-five scenarios (S1–S44 plus the S6b variant) cover search, details, downloads, source selection, error
...
The table below is a single live run of the full suite against `claude-haiku-4-5`
(real Anthropic API, real mirrors, real downloads): **42 passed, 0 failed, 0
skipped** — every scenario in the suite as it stood, including the 10 that run
against a server in remote (`--http`) mode.

S42–S44 were added after that run and are not in the table below. Each was
verified live on its own; they will appear here after the next full run.

A clean sweep is worth reading carefully rather than celebrating. It is the first
one this suite has produced, and it took removing the two things that were hiding
trouble: scenarios that skipped on a live-data condition, and assertions that
graded one arbitrary attempt instead of what the model achieved. A skip that
recurs is not a passing test, it is an unasked question — and the first one this
suite stopped skipping turned out to be a model inventing a table of contents for
