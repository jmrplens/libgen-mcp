...
head:
  ...
  # Measurement results, so Dataset is the right type. No `distribution`: the
  # numbers are generated into this page from the run report, and there is no
  # machine-readable artifact to point at.
  - tag: script
    attrs:
      type: application/ld+json
    content: |
      {
        ...
        "measurementTechnique": "Live end-to-end MCP session against a real language model, real mirrors and real downloads",
        "variableMeasured": [
          {
            "@type": "PropertyValue",
            "name": "Tool selection",
