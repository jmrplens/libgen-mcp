...
{/* prettier-ignore */}
The suite is **76 scenarios** (S1–S75 plus the S6b variant). 10 of them drive a server in remote (`--http`) mode; the rest run it over stdio.
...
{/* prettier-ignore */}
The table below is a single live run of the full suite on 2026-07-30 against `claude-haiku-4-5-20251001` (real Anthropic API, real mirrors, real downloads): **76 passed, 0 failed, 0 skipped** out of 76 — every scenario, including the 10 that run against a server in remote (`--http`) mode.
