...
{/* prettier-ignore */}
The suite is **78 scenarios** (S1–S77 plus the S6b variant). 10 of them drive a server in remote (`--http`) mode; the rest run it over stdio.
...
{/* prettier-ignore */}
The table below is a single live run of the full suite on 2026-07-30 against `claude-haiku-4-5-20251001` (real Anthropic API, real mirrors, real downloads): **76 passed, 0 failed, 0 skipped** out of the 76 measured so far, including the 10 that run against a server in remote (`--http`) mode. 2 scenarios in the list above have no row here yet: they were added after the last live run, and a result is published only once it has been measured.
