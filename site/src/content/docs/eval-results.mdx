...
{/* prettier-ignore */}
The table below is assembled from live runs between 2026-07-25 and 2026-07-30 against `claude-haiku-4-5-20251001` (real Anthropic API, real mirrors, real downloads): **71 passed, 0 failed, 0 skipped** out of 71 — every scenario, including the 10 that run against a server in remote (`--http`) mode. Each row carries the date it was last measured: a partial run refreshes only the scenarios it executed.
