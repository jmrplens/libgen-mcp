...
{/* prettier-ignore */}
The suite is **81 scenarios** (S1–S80 plus the S6b variant). 10 of them drive a server in remote (`--http`) mode; the rest run it over stdio.
...
{/* prettier-ignore */}
The table below is assembled from live runs between 2026-08-08 and 2026-08-09 against `claude-haiku-4-5-20251001` (real Anthropic API, real mirrors, real downloads): **77 passed, 4 failed, 0 skipped** out of 81 — every scenario, including the 10 that run against a server in remote (`--http`) mode. Each row carries the date it was last measured: a partial run refreshes only the scenarios it executed.
