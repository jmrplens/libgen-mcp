...
chips:
  - text: "Live, graded run"
  - text: "Real mirrors"
  - text: "Unguided prompts"
  - text: "Tools"
    href: "/libgen-mcp/tools/"
datePublished: "2026-07-21"
head:
  - tag: title
    content: "libgen-mcp evaluation: how a real model uses the tools"
    ...
    content: |
      {
        ...
        "mainEntity": [
          ...
          },
          {
            ...
            "acceptedAnswer": {
              "@type": "Answer",
              "text": "That there was nothing to grade: an optional source the scenario is about contributed nothing that day, a precondition did not arise, a capability was not configured, or the model ran out of turns. It is never a model failure, and a payload that fails to arrive is not a skip: those runs are graded on whether the answer says nothing came back. Skips are treated with suspicion rather than relief: a scenario that skips often is an unasked question, and removing the skips is what exposed a model inventing a table of contents for a file it could not read."
            }
          }
        ]
      }
    ...
    content: |
      {
        ...
        "keywords": "Model Context Protocol, MCP server evaluation, LLM tool use, Library Genesis, open access, Claude",
...
Each scenario returns **PASS**, **FAIL**, or **SKIP**. A SKIP means there was
nothing to grade: an optional source the scenario is about contributed nothing
that day, a precondition the scenario needs did not arise, a capability the
deployment has not configured, or a model that ran out of turns before
answering. It is never a model failure.
...
{/* prettier-ignore */}
The suite is **81 scenarios** (S1–S80 plus the S6b variant). 10 of them drive a server in remote (`--http`) mode; the rest run it over stdio. **28 are coached**: the prompt names the source, the collection or the field the assertion then checks, so what they measure is that the server obeys rather than that it explains itself. The rest hand the model nothing but the need.
...
importantly — **uncoached** prompts that give the model no hints, testing whether
the tool surface is self-describing enough for an LLM to use unaided. The
stimulus column on every row says which kind it is, so a coached measurement is
never read as an uncoached one.
...
What the remote block grades is the link: the model must call `download` and the
server must hand back a URL the harness can fetch. Whether the host at the far
end then serves that harness is the host's own decision — a mirror returning 500,
a publisher bouncing an unbranded client through a redirect loop — and neither is
behavior this project controls, so a refused fetch is recorded in the row as
evidence and does not fail the scenario. A run in which no fetchable link came
back at all does fail it. S27–S31, and the
...
transport, eight scenarios in all. With S17 and S18, ten scenarios run over HTTP.
...
{/* prettier-ignore */}
The table below is a single live run of the full suite on 2026-09-22 against `claude-haiku-4-5-20251001` (real Anthropic API, real mirrors, real downloads): **78 passed, 0 failed, 3 skipped** out of 81 — every scenario, including the 10 that run against a server in remote (`--http`) mode. **28 of the 81 measured are coached** — their prompt named the source, the collection or the field the assertion checks — so they say the server obeys, not that it describes itself well enough to be used unaided. The scenario table above marks every one.

{/* end generated */}

Nine of those passes grade an absence rather than a result. S32–S37, S40, S41
and S77 exercise Anna's Archive, whose HTML site answered every request with a
browser challenge on the day of the run, so what they measured is that the model
said plainly that no Anna's result came back instead of inventing one. They are
passes on honesty, and say nothing about the Anna's path itself.
...
says plainly that nothing came back. SKIP is reserved for the cases with genuinely
nothing to grade: an optional source the scenario is about contributing nothing
(the three skips in the run above are Anna's Archive, Project Gutenberg and dblp
returning no hits of their own), a precondition that did not arise, a capability
the deployment has not configured, and a model that ran out of turns before
answering.
