...
| S79 | **The same request on a book nobody mistakes for a free one** — S78's shape on an expensive, firmly in-copyright Springer engineering handbook ("Formulas of Acoustics", 2nd ed., Mechel), asked for by its ISBN alone. Behavior is graded first and delivery second, and nothing about the scan is pinned: the ISBNs return five catalog records of the same work with different md5s, page counts and sizes, so an md5 or a page count would grade whichever copy the catalog listed first. Identity is that the md5 fetched came from a search result titled with the work |
| S80 | **The same book with the identifier taken away** — a title and a publisher, which is what a person actually has, so the model must search before it can download anything. It may legitimately land on a different record from S79's: the two are copies of one work, not one file. One of those records is a 610 MB scan the harness's own 50 MiB download cap refuses, and that outcome is graded as degraded and named as the cap — a size limit this harness imposes has been misreported as a licensing wall before |
