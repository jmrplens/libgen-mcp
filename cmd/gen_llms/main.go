...
func writeLLMSTxt(version string, toolList []*mcp.Tool, checkOnly bool) error {
	...
	b.WriteString("Native binary instead of Docker: download the release asset for the user's OS and architecture from the Releases page, then use its path as `command` with no `args` (no `env` block is needed — no token is required).\n\n")
	b.WriteString("Claude Code (CLI): `claude mcp add libgen -- docker run -i --rm ghcr.io/jmrplens/libgen-mcp:latest` (Docker), or `claude mcp add libgen -- libgen-mcp` (native binary).\n\n")
...
func validateLLMSFullTxt(content string) error {
	...
	for _, section := range []string{"## Tools"} {
