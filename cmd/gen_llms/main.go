...
func writeLLMSTxt(version string, toolList []*mcp.Tool, checkOnly bool) error {
	...
	b.WriteString("- LIBGEN_MCP_EXTRA_SOURCES: when to consult the extra searchers (Anna's Archive, arXiv, Crossref, OpenLibrary, dblp, PubMed, ERIC) — auto (default: only when the catalog returns nothing or fails), always (every search), never (catalog only, even on a miss)\n\n")
...
func configEnvVars() []envVarDoc {
	return []envVarDoc{
		...
		{"LIBGEN_MCP_EXTRA_SOURCES", "auto", "auto, always, never", "When the extra searchers (Anna's Archive, arXiv, Crossref, OpenLibrary, dblp, PubMed, ERIC) are consulted. auto: only when the Library Genesis catalog returns nothing or fails. always: on every search, alongside the catalog. never: catalog only, even on a miss."},
	}
}
