...
func writeLLMSTxt(version string, toolList []*mcp.Tool, checkOnly bool) error {
	...
	b.WriteString("- LIBGEN_MCP_EXTRA_SOURCES: when to consult the extra searchers (Anna's Archive, arXiv, Crossref, OpenLibrary, Project Gutenberg, dblp, PubMed) — auto (default: only when the catalog returns nothing or fails), always (every search), never (catalog only, even on a miss)\n\n")
...
func writeLLMSFullDownloadSources(b *strings.Builder) {
	...
	b.WriteString("- **Articles (by `doi`):** the open-access providers first — `unpaywall` (requires `LIBGEN_MCP_UNPAYWALL_EMAIL`), then `europepmc` (Europe PMC open-access full text), then `biorxiv` (`10.1101` preprints), then `fatcat` (Internet Archive Scholar), then `core` (requires `LIBGEN_MCP_CORE_KEY`), then `oapen` (monograph DOIs) — then the shadow-library fallbacks `scihub` (`LIBGEN_MCP_SCIHUB_HOSTS`) and `scidb` (Anna's Archive SciDB viewer).\n")
