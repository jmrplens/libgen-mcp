...
func writeLLMSFullTransports(b *strings.Builder) {
	...
	b.WriteString("- **Flags for environment settings:** `--log-level`, `--download-dir`, `--mirror`, `--sources`, `--allow-private-addresses` and `--env-file` each write their variable, and only when passed, so an exported value survives a flag that was not typed. The credential-shaped settings (`LIBGEN_MCP_ANNAS_KEY`, `LIBGEN_MCP_CORE_KEY`, `LIBGEN_MCP_UNPAYWALL_EMAIL`) deliberately have no flags: a secret on a command line is visible through `ps` and lands in shell history.\n")
