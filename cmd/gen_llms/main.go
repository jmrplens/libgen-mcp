...
func configEnvVars() []envVarDoc {
	return []envVarDoc{
		...
		{"LIBGEN_MCP_DOWNLOAD_RETRY_EVERY_SOURCE", "false", "1/true/0/false", "Give every download source the full start-retry schedule. By default the schedule is spent only on the last source that can serve the item, so a source that is down does not hold up one that is not."},
		...
		{"LIBGEN_MCP_ENRICH", "true", "1/true/0/false", "Deployment kill-switch for get_details' opt-in Crossref/OpenLibrary enrichment. Default true only allows it — a call still has to pass enrich: true. Set false to forbid it entirely."},
