...
func writeLLMSFullTransports(b *strings.Builder) {
	...
	b.WriteString("- **No server-side fetching:** a remote deployment also does not pull file bytes, so it serves three tools rather than four — `read` is not registered, because returning one page means fetching the whole file over an egress IP shared by every user of the deployment. Clients fetch the link `download` returns and read the file themselves. `LIBGEN_MCP_SERVER_FETCH=1` turns it back on; link resolution, `search` and `get_details` are never gated.\n\n")
}
