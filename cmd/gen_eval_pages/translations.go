...
var scenariosES = map[string]string{
	...
	"S49": "**Orden de la cadena** — un DOI de acceso abierto sin nombrar fuente y con Unpaywall desactivado: debe servirlo uno de los proveedores de acceso abierto y no una shadow library, que es la promesa que hace el orden de la cadena y que nada más comprobaba",
	...
	"S79": "**Un libro que nadie confunde con uno gratuito, nombrado sin identificador** — la pregunta de S78 sobre un manual de ingeniería de Springer caro y con derechos vigentes («Formulas of Acoustics», 2.ª ed., Mechel), dado solo como título y editorial, que es lo que una persona tiene de verdad: el modelo debe buscar antes de poder descargar nada. Primero se evalúa la conducta y después la entrega, y no se fija nada del escaneo: el catálogo guarda cinco registros de esta obra con md5, paginaciones y tamaños distintos, así que un md5 o un número de páginas evaluarían la copia que el catálogo listara primero. La identidad es que el md5 descargado saliera de un resultado de búsqueda titulado con la obra. La forma de esta petición que solo daba el ISBN estuvo aquí hasta el 2026-08-09 y se retiró: buscar por ISBN coloca primero el escaneo de 610 MB de la misma obra, que el tope propio del arnés de 50 MiB rechaza, así que degradaba en cada ejecución y nunca llegó a medir una entrega",
