...
func (c *Client) DownloadItem(ctx context.Context, item Item, dir, filename string, progress ...ProgressFunc) (*DownloadResult, error) {
	...
	if len(errs) == 0 {
		if item.Source != "" {
			return nil, fmt.Errorf("source %q cannot serve md5=%q doi=%q (md5 uses libgen/randombook/annas; doi uses unpaywall/europepmc/biorxiv/fatcat/core/scihub/scidb)", item.Source, item.MD5, item.DOI)
		}
