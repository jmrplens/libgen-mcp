...
type ResolvedLink struct {
	...
	Source    string            `json:"source" jsonschema:"the source that resolved the URL: libgen, randombook or annas for books by md5; oapen or archive for books by isbn; unpaywall, europepmc, biorxiv, rfc, nist, dagstuhl, acl, zenodo, fatcat, core, oapen, scihub or scidb for articles by doi"`
...
type DownloadInput struct {
	...
	Source      string `json:"source,omitempty" jsonschema:"restrict the download to a single source instead of trying all: libgen, randombook or annas for books (md5); oapen or archive for books (isbn); unpaywall, europepmc, biorxiv, rfc, nist, dagstuhl, acl, zenodo, fatcat, core, oapen, scihub or scidb for articles (doi). rfc, nist, dagstuhl, acl and zenodo each serve only their own publisher's DOIs. unpaywall needs LIBGEN_MCP_UNPAYWALL_EMAIL and core needs LIBGEN_MCP_CORE_KEY, so both are absent unless configured. Omit to try every compatible source in order with failover"`
