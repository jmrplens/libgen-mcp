...
The order comes from `config.KnownSources` and is fixed in code: for an article, the legal
open-access providers lead, and the shadow libraries are reached only after all of them have
declined. A book asked for by md5 has no legal source to try first, since none is keyed by md5.
...
OpenAlex is OurResearch's open catalogue of scholarly works, built on the same open-access
index that backs Unpaywall — and reachable without a credential of any kind.

- **Keyed by** any DOI — no prefix restriction.
- **Corpus** the same open-access locations Unpaywall reports, since both are OurResearch
  projects reading one index. Measured against twelve DOIs across Oxford, Elsevier, Frontiers,
  Springer, Wiley, Copernicus, MDPI, Cambridge, JSTOR and Routledge, the two returned an
  identical best location every time. This source is therefore **not** here to widen coverage
  over Unpaywall; it is here because it needs no email, so it covers that ground on a keyless
  deployment, where `unpaywall` declines to call its API at all.
- **How it resolves** one GET to `api.openalex.org/works/doi:<doi>`, projected with
  `select=open_access,best_oa_location,locations` so the response omits the authorships,
  topics and citation lists that dwarf the part actually read. The DOI's slashes stay literal
  in the path; other URL-unsafe characters are percent-encoded, and the identifier is matched
  case-insensitively by the API. From the record it takes `best_oa_location.pdf_url`, else the
  first open-access location that is the published version and carries a `pdf_url`, else any
  open-access location's `pdf_url`. A location with `is_oa: false` is skipped — OpenAlex lists
  the publisher's paywalled record for every work — and every candidate must be an absolute
  http(s) URL before it reaches the pipeline.
- **What it does not cover** a record with no `pdf_url` anywhere yields nothing. There is
  deliberately no landing-page fallback: across the records that reach that branch the landing
  URL is a `doi.org` resolver or a repository splash page answering HTML, which the pipeline
  would store as a `.pdf`. Nine such records were checked and not one resolved to a PDF, so
  the fallback would have bought recall it does not actually have. The address OpenAlex names
  is also the publisher's, not its own: Wiley and Taylor & Francis among others answer an
  automated request with 403 HTML, which is a resolved-but-unfetchable outcome rather than a
...
- **Measured 2026-08-21: the article page is behind a browser check.** `scielo.br` answers 403
  to this project's User-Agent and to a bare `curl` alike, and serves an "Establishing a secure
  connection" interstitial to a third-party fetcher, so the block is not aimed at one address.
  The DOI still resolves to the correct article URL, so the source is calling the right place;
  it is the page that is now closed to automated clients.
...
- **Where it sits** last of the article-specific resolvers, after the open-access indexes and
  the archives: a freely licensed copy is preferred wherever one exists, but a file the
  publisher itself serves openly is always tried before a shadow library. Only the two book
  sources (`oapen`, `archive`) and then the shadow libraries follow it.
- **Keys** keyless.
...
- **What it refuses** lending-restricted scans, and the gate is double because neither half is
  ...
  *looks* like success. So the work must be both `ebook_access: public` and `has_fulltext` to
  OpenLibrary **and** the
...
For an article, everything below is reached only after every source above has declined, which
is the whole point of the chain order: a freely licensed copy is always preferred when one
exists, and the order is fixed in code and held by a test rather than left to configuration. A
book asked for by md5 is different: no source above is keyed by md5, so `libgen`, `randombook`
and `annas` are the only ones that can serve it, and Library Genesis is the first it meets.
