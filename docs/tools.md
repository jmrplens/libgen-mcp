...
- **Article** (`doi` only) → the legal open-access providers first — `unpaywall` (only when
  ...
  (`10.1101` preprints), `fatcat`, `core` (only when `LIBGEN_MCP_CORE_KEY` is set) and `oapen`
...
| `resolved.source`     | string | The source that resolved it (`libgen`, `randombook`, `annas`, `oapen`, `archive`, `unpaywall`, `europepmc`, `biorxiv`, `fatcat`, `core`, `scihub`, `scidb`). |
...
| `source`      | string | no       | Restrict the fetch to one source (`libgen`/`randombook`/`annas` for `md5`; `unpaywall`/`europepmc`/`biorxiv`/`fatcat`/`core`/`oapen`/`scihub`/`scidb` for `doi`). `unpaywall` needs `LIBGEN_MCP_UNPAYWALL_EMAIL` and `core` needs `LIBGEN_MCP_CORE_KEY`. |
