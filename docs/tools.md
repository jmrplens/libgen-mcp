...
| `source`            | string | no       | Restrict the download to a single source: `libgen`/`randombook`/`annas` (books, `md5`), `oapen`/`archive` (books, `isbn`) or `unpaywall`/`europepmc`/`biorxiv`/`rfc`/`nist`/`dagstuhl`/`acl`/`zenodo`/`scielo`/`fao`/`fatcat`/`core`/`oapen`/`scihub`/`scidb` (articles, `doi`). `unpaywall` is only selectable when `LIBGEN_MCP_UNPAYWALL_EMAIL` is set, and `core` only when `LIBGEN_MCP_CORE_KEY` is set. Omit to try every compatible source in order with failover. |
...
| `skip_confirmation` | bool   | no       | When `true`, save the file without asking the user to confirm first. Only set it when the user has already agreed or has asked not to be prompted. No effect when the server runs with `LIBGEN_MCP_CONFIRM_DOWNLOADS=false`, or when the client cannot be prompted at all.                                                                                                                                                                                               |
...
- **Article** (`doi` only) → the legal open-access providers first — `unpaywall` (only when
  ...
  `fatcat`, `core` (only when `LIBGEN_MCP_CORE_KEY` is set) and `oapen`
...
| `source`            | string | The source that succeeded: `libgen`, `randombook`, `annas` (books by md5), `oapen`, `archive` (books by isbn) or `unpaywall`, `europepmc`, `biorxiv`, `rfc`, `nist`, `dagstuhl`, `acl`, `zenodo`, `scielo`, `fao`, `fatcat`, `core`, `oapen`, `scihub`, `scidb` (articles).                                              |
...
| `resolved.source`     | string | The source that resolved it (`libgen`, `randombook`, `annas`, `oapen`, `archive`, `unpaywall`, `europepmc`, `biorxiv`, `rfc`, `nist`, `dagstuhl`, `acl`, `zenodo`, `scielo`, `fao`, `fatcat`, `core`, `scihub`, `scidb`). |
...
- **Download confirmation.** Before `download` writes a file to the server's disk (i.e. not
  ...
  tick it and later downloads in that session save without prompting. Two other opt-outs do the
  same job at different scopes — `skip_confirmation: true` for a single call, and
  `LIBGEN_MCP_CONFIRM_DOWNLOADS=false` for the whole deployment. Any one of them is enough, and
...
| `source`      | string | no       | Restrict the fetch to one source (`libgen`/`randombook`/`annas` for `md5`; `unpaywall`/`europepmc`/`biorxiv`/`rfc`/`nist`/`dagstuhl`/`acl`/`zenodo`/`scielo`/`fao`/`fatcat`/`core`/`oapen`/`scihub`/`scidb` for `doi`). `unpaywall` needs `LIBGEN_MCP_UNPAYWALL_EMAIL` and `core` needs `LIBGEN_MCP_CORE_KEY`. |
