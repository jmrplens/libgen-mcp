...
[Project Gutenberg](https://www.gutenberg.org/), [dblp](https://dblp.org/), and [PubMed](https://pubmed.ncbi.nlm.nih.gov/) — controlled by the `extra_sources` argument (`auto`/`always`/`never`) and its deployment
...
`origin: "annas"`); arXiv, Crossref, OpenLibrary, Project Gutenberg, dblp, and PubMed hits appear in a separate
`open_access` field alongside `results`.

That field name predates the two bibliographic indexes and is now a slight misnomer: dblp and
PubMed describe a paper without asserting anything about its availability, so **only a hit whose
own `open_access` flag is `true` is known to be free to read**. A dblp or PubMed hit is a
citation, not a download.
...
dblp's conference or journal abbreviation (`DAC`, `Commun. ACM`), or PubMed's full journal name.
...
listed them. All six providers are keyless — no
