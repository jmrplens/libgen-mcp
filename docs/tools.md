...
[dblp](https://dblp.org/), and [PubMed](https://pubmed.ncbi.nlm.nih.gov/) — controlled by the `extra_sources` argument (`auto`/`always`/`never`) and its deployment
...
`origin: "annas"`); arXiv, Crossref, OpenLibrary, dblp, and PubMed hits appear in a separate
