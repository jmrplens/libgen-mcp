...
| `source`            | string | no       | Restrict the download to a single source: `libgen`/`randombook`/`annas` (books, `md5`), `oapen`/`archive` (books, `isbn`) or `unpaywall`/`europepmc`/`biorxiv`/`rfc`/`nist`/`dagstuhl`/`acl`/`zenodo`/`fatcat`/`core`/`oapen`/`scihub`/`scidb` (articles, `doi`). `unpaywall` is only selectable when `LIBGEN_MCP_UNPAYWALL_EMAIL` is set, and `core` only when `LIBGEN_MCP_CORE_KEY` is set. Omit to try every compatible source in order with failover. |
...
- **Article** (`doi` only) → the legal open-access providers first — `unpaywall` (only when
  ...
  papers), `zenodo` (`10.5281/zenodo` deposits),
...
| `source`            | string | The source that succeeded: `libgen`, `randombook`, `annas` (books by md5), `oapen`, `archive` (books by isbn) or `unpaywall`, `europepmc`, `biorxiv`, `rfc`, `nist`, `dagstuhl`, `acl`, `zenodo`, `fatcat`, `core`, `oapen`, `scihub`, `scidb` (articles).                                                               |
...
| `resolved.source`     | string | The source that resolved it (`libgen`, `randombook`, `annas`, `oapen`, `archive`, `unpaywall`, `europepmc`, `biorxiv`, `rfc`, `nist`, `dagstuhl`, `acl`, `zenodo`, `fatcat`, `core`, `scihub`, `scidb`). |
...
| `source`      | string | no       | Restrict the fetch to one source (`libgen`/`randombook`/`annas` for `md5`; `unpaywall`/`europepmc`/`biorxiv`/`rfc`/`nist`/`dagstuhl`/`acl`/`zenodo`/`fatcat`/`core`/`oapen`/`scihub`/`scidb` for `doi`). `unpaywall` needs `LIBGEN_MCP_UNPAYWALL_EMAIL` and `core` needs `LIBGEN_MCP_CORE_KEY`. |
