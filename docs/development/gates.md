...
| `make check-bench-resources` | CI (`Analyze`)                 | The benchmark page, its six figures and the two site pages still say what the record says. It **redraws** them from the committed JSON rather than re-measuring, because a record measured on one machine is not reproducible on a runner and a gate that re-measured would fail for being on different hardware. `make bench-resources` takes the measurement by hand; `make bench-resources-render` redraws it. |
...
| `make check-mcpb`                                        | CI (`server.json`)          | The Claude Desktop bundle starts on every platform it lists: the manifest's platforms and overrides, the Linux launcher under every POSIX shell the runner has, and the packer's refusals over the release job's `dist/`.                               |
