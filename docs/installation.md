...
**What you get.** A `.mcpb` extension bundle — macOS universal, Windows and Linux
(amd64) — that Claude Desktop installs on its own. No Docker, no Node, no `PATH`
to edit. The Linux binary is for other hosts that install MCPB bundles, since
Claude Desktop itself runs on macOS and Windows.

**Install.** Download
[`libgen-mcp.mcpb`](https://github.com/jmrplens/libgen-mcp/releases/latest/download/libgen-mcp.mcpb),
open it with Claude Desktop, and confirm.
