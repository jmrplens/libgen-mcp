...
Which of the two to pick, what the socket's mode means, what happens to a file already at that
path, and how TLS is terminated and renewed are deployment decisions rather than transport
mechanics, so they live on one page of their own:
**[HTTP server mode → Where the server listens](http-server-mode.md#where-the-server-listens)**.
...
**Every flag below also has a variable**, named `LIBGEN_MCP_` plus the flag in upper case with
dashes as underscores — `--http-path` is `LIBGEN_MCP_HTTP_PATH`, `--rate-limit-rps` is
`LIBGEN_MCP_RATE_LIMIT_RPS`. The one exception to the spelling is `--http` itself, whose
variable is `LIBGEN_MCP_HTTP_ADDR`, because `LIBGEN_MCP_HTTP` would read as a switch rather
than an address. The precedence is this server's usual one: **an explicitly passed flag, then
the environment, then the default** — so a compose file's `environment:` block can configure a
...
match its own configuration must not reach production looking healthy. A variable that is set
but blank counts as unset, which is what `${FOO}` expands to when the shell has no `FOO`.
...
| `--http-idle-timeout`       | `0`         | Close a kept-alive connection that has gone this long between requests. `0` closes none, which is what this server did before the flag existed. It bounds the gap between requests and never a response being written, so an SSE stream is not what it reclaims. A negative value fails startup.                                                                                                                       |
| `--session-timeout`         | `30m`       | Close a legacy stateful session that has gone this long without a request from its client; `0` closes none, leaving a client that crashed holding its session for the life of the process. Applies to `--stateless=false` only — under the default stateless transport each POST's session ends with its response, so passing it there fails startup rather than being ignored. Capped at 24 hours.                    |
...
**`/mcp` is an alias for the endpoint**, because enough clients and enough guides assume that
path. Both `/mcp` and `/mcp/` are mounted as exact matches: a bare `/mcp/` would be a
`ServeMux` subtree pattern that swallows every path beneath it, and would also make the mux
answer `/mcp` with a `301` that a POST does not follow with its body. What it means for a
client pasting a base URL is on
[HTTP server mode](http-server-mode.md#the-mcp-alias).
...
machine, reads its listener off its own command line and asks `/health`, and
`libgen-mcp --shutdown` asks every other instance of this binary on the machine to exit. They
share a process lookup, which is why they arrived together: the dependency on a process list is
the cost, and one consumer would not have justified it. What each exits with, and why the image
runs the first as its `HEALTHCHECK` rather than a `curl` line, is on
[HTTP server mode](http-server-mode.md#probing-an-instance-that-is-already-running).

This is **not** `cmd/probe`, which is a maintainer's live diagnostic against the real mirrors.

**`GET /health` answers `200` while the server is serving**, with `status`, `version`, `commit`,
`build`, `config_digest`, `started_at` and `uptime_seconds`. `build` is the one string a display
wants: the release this binary is closest to plus the short commit, so a tag build and a build
from `main` read comparably where `version` alone gives one a plain number and the other a Go
pseudo-version with a timestamp in it. What `config_digest` covers, why a monitor should watch
`started_at`, and how the endpoint behaves on shutdown are on
[HTTP server mode](http-server-mode.md#the-configuration-digest).

**One caller's share is bounded twice**, and both bounds are per charged address: a token bucket
on the methods that reach a mirror or spend this process (`--rate-limit-rps`,
`--rate-limit-burst`), with `tools/list` on a bucket of its own so that draining the call bucket
never costs a caller their discovery; and a ceiling on how many `download` or `read` calls one
caller may have in flight (`--max-inflight-per-client`), with a non-configurable process-wide
ceiling of **64** behind it, since the per-caller one multiplies by however many callers there
are. The methods that reach nothing are not metered at all: `initialize`, `prompts/list`, the
notifications, and the resource methods and `completion/complete`, which already answer `-32601`.

Neither bound is worth more than the identity underneath it, and on two of the three deployment
shapes there is no per-caller identity to bound. That, the figures, the per-address table's
...
reverse-proxy recipe this server is deployed behind: the proxy forwards the client's `Host` and
connects over loopback. What to set so a proxied request is not refused is on
[HTTP server mode](http-server-mode.md#the-name-clients-use-and-why-a-proxied-request-is-refused-without-it).
...
than one per feature, and it is the address the connection came from. Behind a proxy that
address is the proxy's, which `--trusted-proxy-header` and `--trusted-proxies` correct; the
header is read **only** from a listed peer, because from anybody else it is text the caller
wrote. The reasoning is recorded in
[One server for every caller](decisions/2026-09-17-one-server-for-every-caller.md), and what
to set for each deployment shape is on
[HTTP server mode](http-server-mode.md#what-identity-this-deployment-has).
...
Off unless `LIBGEN_MCP_TELEMETRY` says otherwise, and off is the default for privacy rather
than for cost: instrumenting a deployment is a decision about that deployment's own users.
Everything about the exporters — endpoint, headers, protocol, timeouts, sampling, resource
attributes — comes from the standard `OTEL_*` variables the SDK reads itself.
[Configuration](configuration.md#http-listener) lists the variables; this is what the signals
say.

**One span and one duration per MCP request**, recorded by a middleware sitting beneath the
panic guard, so the resource refusal, the cache hints, the per-caller trio and the handler are
all inside one measured request. The span is `SERVER` kind and is parented from the trace
context in `params._meta` rather than from the transport, which is the convention's own
instruction: one MCP request can be served by several HTTP requests when a client retries, and
one streamable HTTP request can carry more than one MCP request, so parenting to the transport
attaches an operation to whichever round trip happened to carry it. The ambient HTTP span is
attached as a **link** instead, so "which request carried this" stays answerable.

**The source chain is the reason it is worth its cost here.** A download that succeeded through
the first source and one that succeeded through the fourth are the same result to a caller and
completely different to whoever has to decide whether a mirror is dying — and the chain is the
only layer that knows. Every attempt is recorded, so `libgen_mcp.source` names the one that
served and `libgen_mcp.source_attempts` says how much of the chain it cost.
`libgen_mcp.mirror_host` names the catalog mirror, as a bare host and never a URL: a resolved
file URL on the member path is itself a working credential.

**A refusal is recorded from where it is decided**, because a refusal travels as a successful
JSON-RPC response carrying a failure meant for the model — from outside the handler it is
indistinguishable from a call that ran and succeeded, which is exactly what it would look like
...
every supporting source is cooled — bypasses the cooldown and tries anyway. Neither declines a
call, so a reason recorded there would mark calls that succeeded as refused.

**Nothing about a caller ever reaches a metric.** Every distinct label combination is a time
series somebody stores and pays for, and a per-caller dimension is unbounded by construction:
on a public endpoint it grows with the internet. The span carries what the identity policy
allows, because a span has no series budget. The same reasoning bounds a name the caller chose:
a `prompts/get` for a prompt that does not exist would otherwise record the invented name as a
...
the port, the scheme and the status, **and never `url.full`**: a download URL frequently carries
the md5 or the DOI in its path, a Sci-Hub or Anna's URL names the item outright, and a resolved
member URL is a working credential. That is the same value the identity policy protects and the
same one the transport-error redaction declines, so shipping it here would be a third door into
one disclosure.

A `4xx` or `5xx` is **not** a span error. A source that does not have an item answers `404` and
the next one serves it, so following the convention's SHOULD there would paint every ordinary
failover red. A transport error is the only failure this layer counts, and its text is not
recorded — `net/http` prints the whole request URL in a `*url.Error`, which would put the URL
back on the span by another route.

The metric bounds the host it names. The mirror list is discovered and rotates, and an
open-access index hands back a publisher's own URL that this server then fetches, so the host on
an outbound request is frequently chosen by a third party; anything outside the set this
deployment is configured to reach is recorded as one bucket, and the port rides with the host so
a bounded address beside an unbounded port cannot move the same cardinality one column over. The
span still carries the real endpoint, because a span has no series budget. A View with an
attribute filter would drop the label in the SDK instead, and is deliberately not used: bounding
the value here means it is never recorded at all, where a filter removes it from the aggregation
and **still lets it reach a collector through exemplars**. A View is a cardinality mechanism,
not a privacy one.

**Nothing propagates outward.** Every outbound request has its `traceparent`, `tracestate` and
`baggage` removed, and baggage is cleared from its context. Both halves are needed: the context
so nothing downstream can inject from it, the headers so an injection that already happened does
not survive. This is where the server differs from its sibling, which keeps `traceparent`
because its single downstream is the instance the operator named. Here the downstreams are
dozens of third parties and several are named _by another third party_ — a publisher link
deposited with Crossref, a repository URL republished by an open-access index, a
`citation_pdf_url` scraped off a page — so a `traceparent` injected toward one hands a stable
correlation handle to somebody the operator did not choose and cannot audit. The revisit trigger
is an operator-instrumented egress proxy in front of these requests, which would be a downstream
they do choose.

**A session is measured only where a session is a real span of time**: stdio, where it is the
process lifetime, and `--stateless=false`, where it is what an `Mcp-Session-Id` names. Under
the stateless default each POST is its own session, so a session duration would be a histogram
of request durations under a second name. There is a third skip, and it is not obvious: a
stateful transport also creates a throwaway session for the `server/discover` probe a current
client sends before it handshakes, which would put a near-zero sample in for every client that
connects. A session is observed only once it has carried an `initialize`, which is the SDK's
own discriminator — it persists `InitializeParams` only when the transport can serve the
protocol.
