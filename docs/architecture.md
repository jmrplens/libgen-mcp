...
from configuration in the fixed order `unpaywall → europepmc → biorxiv → fatcat → core → scihub → scidb → libgen → randombook → annas`, and each
...
`[libgen, randombook, annas]`, an article item `[unpaywall, europepmc, biorxiv, fatcat, core, scihub, scidb]`, and an item carrying both is
