...
from configuration in the fixed order `unpaywall → europepmc → biorxiv → rfc → nist → fatcat → core → oapen → archive → scihub → scidb → libgen → randombook → annas`, and each
...
`[unpaywall, europepmc, biorxiv, rfc, nist, fatcat, core, oapen, scihub, scidb]`, and an item carrying both
