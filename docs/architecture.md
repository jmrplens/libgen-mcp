...
| `dagstuhl`   | DOI      | Open-access proceedings | Fetches the DROPS document page for a `10.4230` DOI (`drops.dagstuhl.de/entities/document/<doi>`, the URL the DOI resolver itself redirects to) and takes the PDF from its `citation_pdf_url` meta tag. The page has to be read: DROPS storage paths embed the volume the paper appeared in, which the DOI does not carry. Covers LIPIcs, OASIcs, DARTS and the Dagstuhl Reports. Claims only `10.4230` DOIs. Keyless.                                                            |
| `acl`        | DOI      | Open-access proceedings | Builds `aclanthology.org/<id>.pdf` from the identifier embedded in a `10.18653`/`10.3115` `/v1/` DOI. Resolves without a request of its own. A volume-lettered identifier (`N19-1423`) is uppercased and a year-prefixed one (`2024.emnlp-main.856`) left as-is, because the Anthology answers 404 to the other spelling in each case. The pre-2014 numeric `10.3115` form embeds no identifier and is declined. Keyless.                                                         |
| `zenodo`     | DOI      | Open-access deposits    | Lists a `10.5281/zenodo.<id>` record's files through Zenodo's `/api/records/<id>/files` endpoint (the one `/api` path its robots.txt permits) and serves the best entry: a PDF, EPUB or text file if the record holds one, else its largest file. A concept DOI — the identifier for all versions of a deposit, which has no listing of its own — is retried against the version its record page redirects to. Formats are read off the file's name, never its mimetype. Keyless. |
