...
| `core`       | DOI      | Open-access articles  | Looks the DOI up in CORE's v3 API (Bearer-authenticated) and returns the work's download URL. Opt-in: absent from the chain unless `LIBGEN_MCP_CORE_KEY` is set. The key is sent only to `api.core.ac.uk`.                                                             |
