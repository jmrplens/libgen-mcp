...
nist → dagstuhl → acl → zenodo → fatcat → core → oapen → archive → scihub → scidb → libgen →
...
- **Articles** (identified by `doi`) are tried against the legal open-access providers first —
  ...
  Anthology papers) and `zenodo` (Zenodo deposits), then `fatcat`, `core` (only when
