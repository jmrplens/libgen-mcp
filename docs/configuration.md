...
nist → fatcat → core → oapen → archive → scihub → scidb → libgen → randombook → annas`. Each
...
- **Articles** (identified by `doi`) are tried against the legal open-access providers first —
  ...
  `rfc` (RFCs) and `nist` (NIST publications), `fatcat`, `core` (only when `LIBGEN_MCP_CORE_KEY` is set) and `oapen` (monographs carry
