...
| `LIBGEN_MCP_SOURCES`                     | *(all enabled)*                                          | Comma-separated subset of `unpaywall`, `europepmc`, `biorxiv`, `fatcat`, `core`, `scihub`, `scidb`, `libgen`, `randombook`, `annas` | Restrict which download sources are active. Empty means all are enabled. Unknown names are rejected at startup. Order in the variable does not matter — the chain order is fixed (see below).                                                                                                                                                                                                                                                            |
...
fatcat → core → scihub → scidb → libgen → randombook → annas`. Each source declares
which items it supports, so in practice:
...
- **Articles** (identified by `doi`) are tried against the legal open-access providers first —
  ...
  `fatcat`, and `core` (only when `LIBGEN_MCP_CORE_KEY` is set) — then the shadow-library
  fallbacks `scihub` and `scidb`. This prefers a freely licensed copy whenever one exists.
