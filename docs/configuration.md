...
| `LIBGEN_MCP_CONFIRM_DOWNLOADS`           | `true`                                                   | `strconv.ParseBool` values (`1`/`true`/`0`/`false`, etc.)                                                                                               | Ask the user to approve each file `download` writes to disk. Only consulted when the client advertised elicitation. Set `false` to save without prompting — the deployment-wide form of `download`'s `skip_confirmation` argument and of the prompt's "stop asking for this session" checkbox. A non-boolean value fails startup.                                                                                                                                      |
...
The download chain always runs in the fixed order `unpaywall → europepmc → biorxiv →
fatcat → core → oapen → archive → scihub → scidb → libgen → randombook → annas`. Each
...
- **Articles** (identified by `doi`) are tried against the legal open-access providers first —
  ...
  `fatcat`, `core` (only when `LIBGEN_MCP_CORE_KEY` is set) and `oapen` (monographs carry
  DOIs too) — then the shadow-library fallbacks `scihub` and `scidb`. This prefers a freely
  licensed copy whenever one exists.
...
This is the deployment-wide switch. Two narrower opt-outs sit alongside it and any one of them
is enough to skip a given prompt:

- `download`'s `skip_confirmation: true` argument, which suppresses the prompt for one call.
- The **"stop asking for the rest of this session"** checkbox on the prompt itself, which is how
  ...
  client session, so a later connection is asked again; set this variable to `false` when the
  opt-out should survive reconnects.
