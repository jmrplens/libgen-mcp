...
`--log-level`, `--download-dir`, `--mirror`, `--sources`, `--allow-private-addresses`,
`--pprof-addr` and `--env-file`. The credential-shaped settings (`LIBGEN_MCP_ANNAS_KEY`, `LIBGEN_MCP_CORE_KEY`,
...
| `LIBGEN_MCP_ACTION_TIMEOUT`              | `1h`                                                     | `[0, 24h]`, Go duration; `0` disables                                                                                                                                                                                                        | Wall-clock cap on **one tool call**, and the only one there is. `LIBGEN_MCP_RESOLVE_BUDGET` bounds a single source's attempt to resolve a link, and `LIBGEN_MCP_DOWNLOAD_STALL_TIMEOUT` ends a transfer that has stopped — but neither ends one that is merely *slow*, so a mirror trickling a byte just inside the stall window holds a concurrent-download slot, a temp-cache slot and a share of the rate limiter indefinitely. The default is over five times the worst legitimate resolve (the whole retry schedule plus one budget for every source in the chain, about 11½ minutes) and sixty stall windows, so no call that was going to succeed reaches it. A caller's own shorter deadline still wins: this is a ceiling on what the deployment will spend, not a floor on what a client may ask for.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
...
| `LIBGEN_MCP_SOURCES`                     | *(all enabled)*                                          | Comma-separated subset of `unpaywall`, `openalex`, `europepmc`, `biorxiv`, `rfc`, `nist`, `dagstuhl`, `acl`, `zenodo`, `scielo`, `fao`, `fatcat`, `core`, `crossref`, `oapen`, `archive`, `scihub`, `scidb`, `libgen`, `randombook`, `annas` | Restrict which download sources are active. Empty means all are enabled. Unknown names are rejected at startup. Order in the variable does not matter — the chain order is fixed (see below).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| `LIBGEN_MCP_REMOTE_DOWNLOADS`            | `false`                                                  | `strconv.ParseBool` values (`1`/`true`/`0`/`false`, etc.)                                                                                                                                                                                    | Forces the `download` tool into remote mode: it always returns a link (a `resource_link` plus a `resolved` object with any required `headers`) instead of saving a file — the same behavior any `--http` value already triggers, a unix socket path included. Set it for a hosted stdio deployment (e.g. behind `mcp-proxy` on a catalog) whose disk is remote/ephemeral and unreachable by the client. A non-boolean value fails startup.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| `LIBGEN_MCP_SERVER_FETCH`                | `false` on a remote server, `true` on a local stdio one  | `strconv.ParseBool` values (`1`/`true`/`0`/`false`, etc.)                                                                                                                                                                                    | Whether the server may fetch a file's **bytes** over its own connection. With it off the `read` tool is **not registered at all** and `download` only ever returns a link. Off by default on a remote deployment (`--http`, a unix socket, or `LIBGEN_MCP_REMOTE_DOWNLOADS=1`), whose egress IP is shared by every user it serves; on by default on a local stdio server, where the connection and the disk are the user's own. Resolving a link, and `search`/`get_details`, are unaffected. A non-boolean value fails startup.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
...
`LIBGEN_MCP_TIMEOUT` bounds a **single** HTTP request that asks a question. Every such request
has failover behind it — another mirror, another source — so hanging on one is pure loss, which
is why the default is only `10s`. A live run measured three separate requests hanging for the
full 30 seconds the old default allowed, each followed by another mirror that answered
immediately: 90 seconds of nothing. Raise it on a slow link.

`LIBGEN_MCP_RESOLVE_BUDGET` bounds a **whole** resolve: everything one download source does to
turn an item into a direct file URL. That is often several sequential requests, so it defaults
to `30s`. Deriving it from the request timeout, as earlier versions did, coupled the two:
shortening the timeout silently cut every source's resolve budget with it, which can strike a
slow but working multi-hop source out of a chain it would otherwise have served.

Neither applies to the transfer of a file. A stream is governed by the request context and by
the progress-resetting `LIBGEN_MCP_DOWNLOAD_STALL_TIMEOUT`.
...
The download chain always runs in the fixed order `unpaywall → openalex → europepmc → biorxiv → rfc →
nist → dagstuhl → acl → zenodo → scielo → fao → fatcat → core → crossref → oapen → archive → scihub → scidb → libgen →
...
- **Articles** (identified by `doi`) are tried against the legal open-access providers first —
  `unpaywall` (only when `LIBGEN_MCP_UNPAYWALL_EMAIL` is set), then `openalex`, `europepmc`,
  `biorxiv`,
...
- `openalex` looks a DOI up in the [OpenAlex](https://openalex.org) catalogue and takes the
...
The `--http` transport already implies remote-download mode, since an HTTP server answers
clients whose disk it cannot write to. That holds for **every** `--http` value, a unix socket
path included: a socket is served through a reverse proxy, and the clients on the far side of
that proxy are no more reachable than the ones behind a TCP port. It is worth stating because
"unix socket" reads like "local" and is not — the mode is decided by `--http` being set at
all, not by what it is set to.

This variable covers the other case where the same is true: a **stdio** server hosted on a
remote/ephemeral machine (for example, running behind `mcp-proxy` so it can be listed on a
catalog like Glama) whose disk the client cannot reach either. Set
`LIBGEN_MCP_REMOTE_DOWNLOADS=1` on such a deployment and `download` always returns a link
instead of attempting to save a file nobody can retrieve. See
[Tools](tools.md#where-the-file-goes-local-vs-remote) for the resulting output shape.

### `LIBGEN_MCP_SERVER_FETCH`

This decides whether the server may pull a file's **bytes** over its own internet
connection. It is off by default on a remote deployment and on by default on a local
stdio one, and an explicit value wins in either direction.

With it **off**:

- the `read` tool is **not registered**. It disappears from `tools/list` rather than
  appearing and failing every call, because a tool a model can see is a tool it will
  call, and each attempt would cost a turn to learn what the list could have said for
  free. The handshake instructions drop its step too.
- `download` always returns a link (a `resource_link` plus a `resolved` object), even
  on a local server — writing the file to disk is a fetch like any other.
- `search`, `get_details` and `download`'s **link resolution** keep working. See below.

The reason for the remote default is operational, not stylistic. A hosted server reaches
the mirrors over an egress IP shared by every user it serves, so one caller's
multi-megabyte transfers can get that address throttled or blocked and take the
deployment down for everybody. When the client fetches the file instead, a block lands on
whoever provoked it. On a local stdio server the connection and the disk are the user's
own, and fetching the file is exactly what they asked for.

**Resolving a link is deliberately still allowed.** `download` does talk to the mirrors to
produce its URL — LibGen's `key=` token is read off `ads.php`, `randombook` discovers a
mirror first — and `search`/`get_details` query the catalogs outright. All of that stays
on, for three reasons. The property at stake is about sustained multi-megabyte transfers,
not a handful of small HTML and redirect requests, so moving those would buy nothing.
Without resolution `download` could not work at all, leaving a hosted deployment with
nothing to offer. And the catalog queries cannot move client-side either without emptying
the server of its purpose. The rule is therefore precise: **the file body stays on the
client's side of the connection; everything lighter than that does not move.**

Set `LIBGEN_MCP_SERVER_FETCH=1` on a hosted deployment to accept the egress cost and get
`read` back — for example on a private instance, or one whose egress is not shared. Set
`LIBGEN_MCP_SERVER_FETCH=0` on a local server to keep it from fetching files at all.
